...
class Redirector extends CI_Controller {
    ...
    function ver_ficha($chileclic_id){
        ...
        if(!$ficha)
            show_404();
        redirect('http://www.chileatiende.gob.cl/fichas/ver/'.$ficha->id, 'location', 301);
    }
    ...
    function ver_servicio($sigla){
        ...
        if(!$servicio)
            show_404();
        redirect('http://www.chileatiende.gob.cl/servicios/ver/'.$servicio->codigo, 'location', 301);
    }
    
    function portada(){
        redirect('http://www.chileatiende.gob.cl', 'location', 301);
    }
}